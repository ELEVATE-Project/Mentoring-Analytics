...
if (session_attendees_df_fd.count() >=1) :
 ...
 session_attendees_df_fd = session_attendees_df_fd.groupBy("userId").agg(
    F.round(avg(F.col("How would you rate the host of the session?")), 2).alias("How would you rate the host of the session?"),
    F.round(avg(F.col("How would you rate the engagement in the session?")), 2).alias("How would you rate the engagement in the session?")
 )
